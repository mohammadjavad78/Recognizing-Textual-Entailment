...
def test(
    ...
    for batch_idx, (input_ids,token_type_ids,attention_mask, labels) in loop_train:
        ...
        cm+=metrics.confusion_matrix(labels.cpu().detach().numpy(),labels_pred.topk(1,-1)[1][:,0].cpu().detach().numpy(),labels=[1,0,2])
    ...
    cr=classification_report(l1,l2, target_names=['c','e','n'])
    ...
    ax.xaxis.set_ticklabels(['c','e','n']); ax.yaxis.set_ticklabels(['c','e','n'])
    plt.show()
    ...
    if(output_attentions):
        att=custom_model.bert(input_ids)['attentions']
    else:
        att=None
    cr=classification_report(l1,l2, target_names=['c','e','n'])
    ...
    report.to_csv(f"{report_path}/{model_name.split('/')[0]}_report_test.csv")
    return model, optimizer, report,att,input_ids
...
import seaborn
if(output_attentions):
    s=list(trainer[-1][7]).index(102)+1
    ll=custom_model.tokenizer.decode(trainer[-1][7][:list(trainer[-1][7]).index(0)])

    lab=[]
    for i in range(s):
        lab.append(custom_model.tokenizer.decode(trainer[-1][7][i:i+1]))
    for head in range(12):
        sns.set(rc={'figure.figsize':(11.7,8.27)})
        seaborn.heatmap(trainer[-2][-1].cpu().detach()[7][head][:s,:s],xticklabels=lab,yticklabels=lab)
        plt.show()